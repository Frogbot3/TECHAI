...
function CategoryItemButton({
  ...
  return (
    <button
      ...
      className={`flex-shrink-0 flex flex-col items-center justify-center p-3 rounded-2xl border transition-all duration-200 cursor-pointer snap-start w-28 sm:w-32 text-center group ${
        isSelected
          ? "bg-cyan-50 border-cyan-500 shadow-xs"
          : "bg-white border-slate-200 hover:border-slate-300 hover:bg-slate-50/70"
      }`}
    >
      <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-center p-1.5 mb-2 overflow-hidden group-hover:scale-105 transition-transform duration-200">
        {!imgError && cat.image ? (
          <img
            src={cat.image}
            alt={cat.name}
      ...
      <div className="flex items-center justify-between mb-3.5">
        <div>
          ...
          <p className="text-xs text-slate-500 font-medium">
            Explore products by curated departments
          </p>
        </div>
